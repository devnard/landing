$(function(){
    characters.map(function(character){
        console.log("Targeting chart for:", character.type);
        var chart = $('.' + type + '.radar-chart');
        chart.radarChart({
            size: [500, 350],
            step: 1,
            title: "",
            values: {
              "character": 5,
              "front-end": Number(character.stats.front_end),
              "back-end": Number(character.stats.back_end),
              "client-side": Number(character.stats.client_side),
              "server-side": Number(character.stats.server_side),
              "ux-mana": Number(character.stats.ux_mana),
              "ui-mana": Number(character.stats.ui_mana),
            },
            showAxisLabels: true
        });
    })
...
(function($) {
  ...
  var Radar = (function() {
    ...
    Radar.prototype.draw = function() {
      ...
      this.cxt.font = "bold 15px Verdana";
      this.cxt.fillText(this.settings.title, 205, 15); 
    }
